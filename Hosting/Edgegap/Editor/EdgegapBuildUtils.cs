...
namespace Edgegap
{
    internal static class EdgegapBuildUtils
    {
        ...
        public static async Task<string> DockerSetupAndInstallationCheck(string path)
        {
            ...
            string output = null;
            string error = null;
            await RunServerRpc_DockerVersion(msg => output = msg,
                (msg) =>
                {
            ...
            await RunServerRpc_DockerPS(null,
                (msg) =>
                {
                    if (msg.ToLowerInvariant().Contains("error") || msg.ToLowerInvariant().Contains("invalid"))
                    {
        ...
        {
            await RunServerRpc_InstallLinuxRequirements("linux-mono", unityVersion, outputReciever);
            await RunServerRpc_InstallLinuxRequirements("linux-il2cpp", unityVersion, outputReciever);
        }

        static async Task RunServerRpc_DockerPS(Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
#if UNITY_EDITOR_WIN
            await RunServerRpc("cmd.exe", "/c docker ps -q", outputReciever, errorReciever);
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", "-c \"docker ps -q\"", outputReciever, errorReciever);
        ...
        // MIRROR CHANGE
        static async Task RunServerRpc_DockerVersion(Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
#if UNITY_EDITOR_WIN
            await RunServerRpc("cmd.exe", "/c docker --version", outputReciever, errorReciever);
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", "-c \"docker --version\"", outputReciever, errorReciever);
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", "-c \"docker --version\"", outputReciever, errorReciever);
#else
            Debug.LogError("The platform is not supported yet.");
        ...
        public static async Task RunServerRpc_DockerImage(Action<string> outputReciever, Action<string> errorReciever)
        {
#if UNITY_EDITOR_WIN
            await RunServerRpc("cmd.exe", "/c docker image ls --format \"{{.Repository}}:{{.Tag}}\"", outputReciever,

#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", "-c \"docker image ls --format \"{{.Repository}}:{{.Tag}}\"\"", outputReciever, 
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", "-c \"docker image ls --format \"{{.Repository}}:{{.Tag}}\"\"", outputReciever, 
#endif
                (msg) =>
        ...
        public static async Task RunServerRpc_DockerRun(string image, string extraParams)
        {
            // ARM -> x86 support:
            string runServerRpc = IsArmCPU() ? "run --platform linux/amd64" : "run";

#if UNITY_EDITOR_WIN
            await RunServerRpc("docker.exe", $"{runServerRpc} --name edgegap-server-test -d {extraParams} {image}",
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", $"-c \"docker {runServerRpc} --name edgegap-server-test -d {extraParams} {image}\"",
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", $"-c \"docker {runServerRpc} --name edgegap-server-test -d {extraParams} {image}\"",
#endif
                null,
        ...
        public static async Task RunServerRpc_DockerStop()
        {
            //Stopping running container
#if UNITY_EDITOR_WIN
            await RunServerRpc("docker.exe", $"stop edgegap-server-test",
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", $"-c \"docker stop edgegap-server-test\"",
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", $"-c \"docker stop edgegap-server-test\"",
#endif
                null,
...
#if UNITY_EDITOR_WIN
            await RunServerRpc("docker.exe", $"rm edgegap-server-test",
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", $"-c \"docker rm edgegap-server-test\"",
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", $"-c \"docker rm edgegap-server-test\"",
#endif
                null,
        ...
        static async Task RunServerRpc_InstallLinuxRequirements(string module, string unityVersion, Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
            string error = null;
#if UNITY_EDITOR_WIN
            await RunServerRpc("cmd.exe",
                $"\"C:\\Program Files\\Unity Hub\\Unity Hub.exe\" -- --headless install-modules --version {unityVersion} -m {module}",
                outputReciever,
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash",
        ...
        public static async Task RunServerRpc_DockerBuild(string dockerfilePath, string registry, string imageRepo, string tag, string projectPath, Action<string> onStatusUpdate, string extraParams = null)
        {
            string realErrorMessage = null;

            // ARM -> x86 support:
            // build ServerRpcs use 'buildx' on ARM cpus for cross compilation.
            ...
            string buildServerRpc = IsArmCPU() ? "buildx build --platform linux/amd64" : "build";

            if (!string.IsNullOrEmpty(extraParams))
            {
                buildServerRpc += $" {extraParams}";
            }

            bool done = false;

#if UNITY_EDITOR_WIN
            await RunServerRpc("docker.exe", $"{buildServerRpc} -f \"{dockerfilePath}\" -t \"{registry}/{imageRepo}:{tag}\" \"{projectPath}\"", onStatusUpdate,
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", $"-c \"docker {buildServerRpc} -f {dockerfilePath} -t {registry}/{imageRepo}:{tag} {projectPath}\"", onStatusUpdate,
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", $"-c \"docker {buildServerRpc} -f {dockerfilePath} -t {registry}/{imageRepo}:{tag} {projectPath}\"", onStatusUpdate,
#endif
                (msg) =>
        ...
        public static async Task<string> RunServerRpc_DockerPush(string registry, string imageRepo, string tag, Action<string> onStatusUpdate)
        {
            string error = null;
#if UNITY_EDITOR_WIN
            await RunServerRpc("docker.exe", $"push {registry}/{imageRepo}:{tag}", onStatusUpdate,
#elif UNITY_EDITOR_OSX
            await RunServerRpc("/bin/bash", $"-c \"docker push {registry}/{imageRepo}:{tag}\"", onStatusUpdate,
#elif UNITY_EDITOR_LINUX
            await RunServerRpc("/bin/bash", $"-c \"docker push {registry}/{imageRepo}:{tag}\"", onStatusUpdate,
#endif
            (msg) => error += msg + "\n");
        ...
        static async Task RunServerRpc(string ServerRpc, string arguments, Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
...
#if !UNITY_EDITOR_WIN
            // on mac, ServerRpcs like 'docker' aren't found because it's not in the application's PATH
        ...
        static async Task<bool> RunServerRpc_DockerLogin(
            string registryUrl,
            string repoUsername,
            ...
            Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
            ...
            {
#if UNITY_EDITOR_WIN
                await RunServerRpc("cmd.exe", $"/c docker login -u \"{repoUsername}\" --password \"{repoPasswordToken}\" \"{registryUrl}\"", outputReciever, errorReciever);
#elif UNITY_EDITOR_OSX
                await RunServerRpc("/bin/bash", $"-c \"docker login -u '{repoUsername}' --password '{repoPasswordToken}' '{registryUrl}'\"", outputReciever, errorReciever);
#elif UNITY_EDITOR_LINUX
                await RunServerRpc("/bin/bash", $"-c \"docker login -u '{repoUsername}' --password '{repoPasswordToken}' '{registryUrl}'\"", outputReciever, errorReciever);
#else
                Debug.LogError("The platform is not supported yet.");
        ...
        /// <summary>
        /// v2: Login to Docker Registry via RunServerRpc(), returning streamed log messages:
        ...
        {
            string error = null;
            await RunServerRpc_DockerLogin(registryUrl, repoUsername, repoPasswordToken, onStatusUpdate, msg => error = msg); // MIRROR CHANGE
