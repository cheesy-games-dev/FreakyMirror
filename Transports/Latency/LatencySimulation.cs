...
namespace Mirror
{
    ...
    public class LatencySimulation : Transport, PortTransport
    {
        ...
        // fixes a bug in prediction where predictedTime would have no latency, but [ServerRpc]s would have 100ms latency resulting in heavy, hard to debug jittering!
        // in real world, all UDP channels go over the same socket connection with the same latency.
        [Tooltip("Latency in milliseconds (1000 = 1 second). Always applied to both reliable and unreliable, otherwise unreliable NetworkTime may be behind reliable [SyncVars/ServerRpcs/Rpcs] or vice versa!")]
