...
namespace Mirror
{
    ...
    public abstract class NetworkBehaviour : MonoBehaviour
    {
        ...
        protected void SendServerRpcInternal(string functionFullName, int functionHashCode, NetworkWriter writer, int channelId, bool requiresAuthority = true)
        {
            ...
            if (!NetworkClient.active)
            {
                Debug.LogError($"ServerRpc {functionFullName} called on {name} without an active client.", gameObject);
                return;
            }
            ...
            if (!NetworkClient.ready)
            {
                ...
                if (channelId == Channels.Reliable)
                    Debug.LogWarning($"ServerRpc {functionFullName} called on {name} while NetworkClient is not ready.\nThis may be ignored if client intentionally set NotReady.", gameObject);
                return;
            }

            // local players can always send ServerRpcs, regardless of authority,
            // other objects must have authority.
            if (!(!requiresAuthority || isLocalPlayer || isOwned))
            {
                Debug.LogWarning($"ServerRpc {functionFullName} called on {name} without authority.", gameObject);
                return;
            }

            // IMPORTANT: can't use .connectionToServer here because calling
            // a ServerRpc on other objects is allowed if requireAuthority is
            ...
            {
                Debug.LogError($"ServerRpc {functionFullName} called on {name} with no client running.", gameObject);
                return;
            }

            if (netId == 0)
            {
                Debug.LogWarning($"ServerRpc {functionFullName} called on {name} with netId=0. Maybe it wasn't spawned yet?", gameObject);
                return;
            }

            // construct the message
            ServerRpcMessage message = new ServerRpcMessage
            {
                netId = netId,
