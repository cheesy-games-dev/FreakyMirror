...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal static void RegisterMessageHandlers()
        {
            ...
            RegisterHandler<ServerRpcMessage>(OnServerRpcMessage);
        ...
        // remote calls ////////////////////////////////////////////////////////
        // Handle ServerRpc from specific player, this could be one of multiple
        ...
        static void OnServerRpcMessage(NetworkConnectionToClient conn, ServerRpcMessage msg, int channelId)
        {
            if (!conn.isReady)
            {
                // Clients may be set NotReady due to scene change or other game logic by user, e.g. respawning.
                // Ignore ServerRpcs that may have been in flight before client received NotReadyMessage message.
                ...
                if (channelId == Channels.Reliable)
                {
                    ...
                    if (spawned.TryGetValue(msg.netId, out NetworkIdentity netIdentity))
                        if (msg.componentIndex < netIdentity.NetworkBehaviours.Length && netIdentity.NetworkBehaviours[msg.componentIndex] is NetworkBehaviour component)
                            ...
                            {
                                Debug.LogWarning($"ServerRpc {methodName} received for {netIdentity.name} [netId={msg.netId}] component {component.name} [index={msg.componentIndex}] when client not ready.\nThis may be ignored if client intentionally set NotReady.");
                                return;
                            }

                    if (RemoteProcedureCalls.GetFunctionMethodName(msg.functionHash, out string method))
                    {
                        Debug.LogWarning($"ServerRpc {method} received from {conn} when client was not ready.\nThis may be ignored if client intentionally set NotReady.");
                        return;
                    }

                    Debug.LogWarning($"ServerRpc received from {conn} while client is not ready.\nThis may be ignored if client intentionally set NotReady.");
                }
                return;
            }
            ...
            {
                // over reliable channel, ServerRpcs should always come after spawn.
                ...
                if (channelId == Channels.Reliable)
                    Debug.LogWarning($"Spawned object not found when handling ServerRpc message netId={msg.netId}");
                return;
            }
            ...
            bool requiresAuthority = RemoteProcedureCalls.ServerRpcRequiresAuthority(msg.functionHash);
            if (requiresAuthority && identity.connectionToClient != conn)
            {
                ...
                if (msg.componentIndex < identity.NetworkBehaviours.Length && identity.NetworkBehaviours[msg.componentIndex] is NetworkBehaviour component)
                    if (RemoteProcedureCalls.GetFunctionMethodName(msg.functionHash, out string methodName))
                    {
                        Debug.LogWarning($"ServerRpc {methodName} received for {identity.name} [netId={msg.netId}] component {component.name} [index={msg.componentIndex}] without authority");
                        return;
                    }

                Debug.LogWarning($"ServerRpc received for {identity.name} [netId={msg.netId}] without authority");
                return;
            }

            // Debug.Log($"OnServerRpcMessage for netId:{msg.netId} conn:{conn}");

            using (NetworkReaderPooled networkReader = NetworkReaderPool.Get(msg.payload))
                identity.HandleRemoteCall(msg.componentIndex, msg.functionHash, RemoteCallType.ServerRpc, networkReader, conn);
        }
        ...
        static void UnSpawnInternal(GameObject obj, bool resetState)
        {
            ...
            if (!active)
            {
                Debug.LogWarning("NetworkServer.Unspawn() called without an active server. Servers can only destroy while active, clients can only ask the server to destroy (for example, with a [ServerRpc]), after which the server may decide to destroy the object and broadcast the change to all clients.");
                return;
            }
        ...
        public static void Destroy(GameObject obj)
        {
            ...
            if (!active)
            {
                Debug.LogWarning("NetworkServer.Destroy() called without an active server. Servers can only destroy while active, clients can only ask the server to destroy (for example, with a [ServerRpc]), after which the server may decide to destroy the object and broadcast the change to all clients.");
                return;
            }
