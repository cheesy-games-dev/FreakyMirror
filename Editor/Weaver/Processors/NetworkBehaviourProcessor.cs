...
namespace Mirror.Weaver
{
    ...
    class NetworkBehaviourProcessor
    {
        ...
        readonly List<MethodDefinition> ServerRpcInvocationFuncs = new List<MethodDefinition>();
        ...
        */
        ...
        {
            if (ServerRpcs.Count == 0 && clientRpcs.Count == 0 && targetRpcs.Count == 0)
                return;
            ...
            // register all ServerRpcs in cctor
            for (int i = 0; i < ServerRpcs.Count; ++i)
            {
                CmdResult cmdResult = ServerRpcs[i];
                GenerateRegisterServerRpcDelegate(cctorWorker, weaverTypes.registerServerRpcReference, ServerRpcInvocationFuncs[i], cmdResult);
            }
        ...
        */
        ...
        // check if a ServerRpc/TargetRpc/Rpc function & parameters are valid for weaving
        ...
        // check if a ServerRpc/TargetRpc/Rpc function is valid for weaving
        ...
        // check if all ServerRpc/TargetRpc/Rpc function's parameters are valid for weaving
        ...
        void ProcessServerRpc(HashSet<string> names, MethodDefinition md, CustomAttribute ServerRpcAttr, ref bool WeavingFailed)
        {
            if (md.IsAbstract)
            {
                Log.Error("Abstract ServerRpcs are currently not supported, use virtual method instead", md);
                WeavingFailed = true;
                return;
            }

            if (!ValidateRemoteCallAndParameters(md, RemoteCallType.ServerRpc, ref WeavingFailed))
                return;
            ...
            MethodDefinition cmdCallFunc = ServerRpcProcessor.ProcessServerRpcCall(weaverTypes, writers, Log, netBehaviourSubclass, md, ServerRpcAttr, ref WeavingFailed);

            MethodDefinition cmdFunc = ServerRpcProcessor.ProcessServerRpcInvoke(weaverTypes, readers, Log, netBehaviourSubclass, md, cmdCallFunc, ref WeavingFailed);
            if (cmdFunc != null)
            {
                ServerRpcInvocationFuncs.Add(cmdFunc);
            }
        }
    }
}
